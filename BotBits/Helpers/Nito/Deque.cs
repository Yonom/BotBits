    ...
    internal sealed class Deque<T> : IList<T>, System.Collections.IList
    {
        ...
        public Deque(IEnumerable<T> collection)
        {
            ...
            if (count > 0)
            {
                ...
                this.DoInsertRange(0, enumerable, count);
            }
            else
            {
                this._buffer = new T[DefaultCapacity];
            }
        }
        ...
        public int Capacity
        {
            ...
            set
            {
                ...
                if (this.IsSplit)
                {
                    ...
                    Array.Copy(this._buffer, this._offset, newBuffer, 0, length);
                    Array.Copy(this._buffer, 0, newBuffer, length, this.Count - length);
                }
                else
                {
                    // The existing buffer is whole
                    Array.Copy(this._buffer, this._offset, newBuffer, 0, this.Count);
                }
        ...
        {
            return this._buffer[this.DequeIndexToBufferIndex(index)];
        }
        ...
        /// </summary>
        /// <param name="value">The value by which to reduce <see cref="_offset"/>. May not be negative or greater than <see cref="_offset"/>.</param>
        ...
        {
            T ret = this._buffer[this.DequeIndexToBufferIndex(this.Count - 1)];
            --this.Count;
            return ret;
        }
        ...
        private void DoInsertRange(int index, IEnumerable<T> collection, int collectionCount)
        {
            ...
            if (index < this.Count / 2)
            {
                ...
                for (int j = 0; j != copyCount; ++j)
                    this._buffer[this.DequeIndexToBufferIndex(writeIndex + j)] = this._buffer[this.DequeIndexToBufferIndex(j)];
            ...
            else
            {
                ...
                for (int j = copyCount - 1; j != -1; --j)
                    this._buffer[this.DequeIndexToBufferIndex(writeIndex + j)] = this._buffer[this.DequeIndexToBufferIndex(index + j)];
            }
            ...
            {
                this._buffer[this.DequeIndexToBufferIndex(i)] = item;
                ++i;
            }
        ...
        private void DoRemoveRange(int index, int collectionCount)
        {
            ...
            else
            {
                ...
                for (int j = 0; j != copyCount; ++j)
                    this._buffer[this.DequeIndexToBufferIndex(index + j)] = this._buffer[this.DequeIndexToBufferIndex(readIndex + j)];
            }
        ...
        public void InsertRange(int index, IEnumerable<T> collection)
        {
            ...
            this.DoInsertRange(index, enumerable, collectionCount);
        }
