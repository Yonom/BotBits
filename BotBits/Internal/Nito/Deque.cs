...
namespace BotBits.Nito
{
    ...
    internal sealed class Deque<T> : IList<T>, IList
    {
        ...
        public int Capacity
        {
            ...
            set
            {
                ...
                if (this.IsSplit)
                {
                    ...
                    var length = this.Capacity - this._offset;
        ...
        {
            var ret = this._buffer[this.DequeIndexToBufferIndex(this.Count - 1)];
            --this.Count;
            return ret;
        }
        ...
        private void DoInsertRange(int index, IEnumerable<T> collection, int collectionCount)
        {
            ...
            if (index < this.Count/2)
            {
                ...
                for (var j = 0; j != copyCount; ++j)
                    this._buffer[this.DequeIndexToBufferIndex(writeIndex + j)] =
                        this._buffer[this.DequeIndexToBufferIndex(j)];
            ...
            else
            {
                ...
                for (var j = copyCount - 1; j != -1; --j)
                    this._buffer[this.DequeIndexToBufferIndex(writeIndex + j)] =
                        this._buffer[this.DequeIndexToBufferIndex(index + j)];
            }
        ...
        private void DoRemoveRange(int index, int collectionCount)
        {
            ...
            else
            {
                ...
                for (var j = 0; j != copyCount; ++j)
                    this._buffer[this.DequeIndexToBufferIndex(index + j)] =
                        this._buffer[this.DequeIndexToBufferIndex(readIndex + j)];
            }
