...
namespace BotBits
{
    public static class BlockSettableExtensions
    {
        ...
        public static void Set<T>(this IEnumerable<T> blocks, Foreground.Id block, string text, string textColor) where T : IBlockSettable<ForegroundBlock, BackgroundBlock>
        {
            blocks.Set(new ForegroundBlock(block, text, textColor));
        }
